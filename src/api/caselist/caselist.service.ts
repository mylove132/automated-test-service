...
export class CaselistService {
    ...
    async findCaseByCaseListId(isTask: boolean, envId: number, options: IPaginationOptions): Promise<Pagination<CaselistEntity>> {
        ...
        let queryBuilder;
        if (isTask && envId) {
            queryBuilder = await this.caseListRepository.createQueryBuilder('caselist').where('caselist.isTask = :isTask', {isTask: isTask}).andWhere('caselist.env = :env', {env: envId}).leftJoinAndSelect('caselist.cases', 'cases').leftJoinAndSelect('cases.endpointObject', 'end').leftJoinAndSelect('caselist.env', 'env').leftJoinAndSelect('env.endpoints', 'endpoint').orderBy('caselist.updateDate', 'DESC');
        } else if (!isTask && envId) {
            queryBuilder = await this.caseListRepository.createQueryBuilder('caselist').andWhere('caselist.env = :env', {env: envId}).leftJoinAndSelect('caselist.cases', 'cases').leftJoinAndSelect('cases.endpointObject', 'end').leftJoinAndSelect('caselist.env', 'env').leftJoinAndSelect('env.endpoints', 'endpoint').orderBy('caselist.updateDate', 'DESC');
        } else if (!isTask && !envId) {
            queryBuilder = await this.caseListRepository.createQueryBuilder('caselist').leftJoinAndSelect('caselist.cases', 'cases').leftJoinAndSelect('cases.endpointObject', 'end').leftJoinAndSelect('caselist.env', 'env').leftJoinAndSelect('env.endpoints', 'endpoint').orderBy('caselist.updateDate', 'DESC');
        } else if (isTask && !envId) {
            queryBuilder = await this.caseListRepository.createQueryBuilder('caselist').where('caselist.isTask = :isTask', {isTask: isTask}).leftJoinAndSelect('caselist.cases', 'cases').leftJoinAndSelect('cases.endpointObject', 'end').leftJoinAndSelect('caselist.env', 'env').leftJoinAndSelect('env.endpoints', 'endpoint').orderBy('caselist.updateDate', 'DESC');
        }
